...
const limitAwards = async (data, field, message, args, get) => {
  ...
  const ticketsCount = await raffle.tickets().getCount()

  const awardsCount = await raffle.awards().getCount()
...
class StoreAward {
  ...
  get messages() {
    return {
      ...
      'description.limitAwards':
        'Limite de premios alcançado. Um premio para cada bilhete da rifa.'
    }
  }
