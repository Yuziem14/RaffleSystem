...
const uniqueUserRaffle = async (data, field, message, args, get) => {
  ...
  const count = (await user.raffles()
    .where('title', value)
    .getCount());
...
class StoreRaffle {
  get rules() {
    ...
    return {
      ...
      probable_raffle_date: `required|date|after:${request.input('start_date_sale')}`,
      end_date_sale: `required|date|before:${request.input('probable_raffle_date')}`,
      type_id: 'required',
  ...
  get messages() {
    return {
      ...
      'start_date_sale.required': 'Por favor, informe a data inicial de vendas.',
      'start_date_sale.date': 'Informe uma data valida.',
      'start_date_sale.after': 'A data inicial de venda deve ser depois da data atual.',
      'probable_raffle_date.required': 'Por favor, informe a data provável do sorteio.',
      'probable_raffle_date.date': 'Informe uma data valida.',
      'probable_raffle_date.after': 'A data provavel do sorteio deve ser depois da data inicial de venda.',
      'end_date_sale.required': 'Por favor, informe a data de fim das vendas.',
      'end_date_sale.date': 'Informe uma data valida.',
      'end_date_sale.before': 'A data final de vendas deve ser antes da data provavél do sorteio.',
      'type_id.required': 'Informe o tipo da rifa.',
      'ticket_price.required': 'Por favor, informe o preço dos bilhetes.',
      'ticket_price.float': 'Por favor, informe um preço válido! O preço deve ser um número.',
