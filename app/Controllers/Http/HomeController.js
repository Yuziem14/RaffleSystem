...
class HomeController {
  ...
  async dashboard({ view, auth }) {
    ...
    for (const raffle of myRaffles) {
      ...
      raffle.available_tickets = await raffle
        .tickets()
        .where('user_id', null)
        .getCount()

      raffle.amountRaised =
        (raffle.totalTickets - raffle.available_tickets) * raffle.ticket_price
      ...
      const probableRaffleDate = moment(
    ...
    for (const raffle of boughtRaffles) {
      ...
      raffle.totalTickets = (
        await raffle.tickets().count('* as total')
      )[0].total
